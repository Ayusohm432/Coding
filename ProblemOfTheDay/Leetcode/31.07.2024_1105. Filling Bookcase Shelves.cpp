...
public:
    ...
    int solve(vector<vector<int>>& books , int i , int remainWidth , int maxHeight){
        if(i >= n){
            return maxHeight;
        }  
        ...
        if(bookWidth <= remainWidth){
            keep = solve(books , i+1 , remainWidth-bookWidth , max(maxHeight , bookHeight));
        }

        skip = maxHeight + solve(books , i+1 , width-bookWidth , bookHeight);

        return dp[i][remainWidth] = min(keep ,skip);
    }
...
int main(){
    Solution solution;
    vector<vector<int>> books = {{1,1},{2,3},{2,3},{1,1},{1,1},{1,1},{1,1}};
    int shelfWidth = 4;
