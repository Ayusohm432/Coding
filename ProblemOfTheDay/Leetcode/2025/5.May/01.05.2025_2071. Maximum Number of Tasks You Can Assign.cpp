...
class Solution {
    bool canAssign(int mid, vector<int>& workers, vector<int>& tasks, int pills, int strength){
        multiset<int> workableWorkers(workers.end()-mid, workers.end());
        for(int i=mid-1; i>=0; i--){
            auto currWorker = --workableWorkers.end();
            ...
            if(*currWorker < tasks[i]){
                ...
                auto weakWorker = workableWorkers.lower_bound(tasks[i] - strength);

                if(weakWorker == workableWorkers.end()){
                    return false;
                }

                pills--;
                workableWorkers.erase(weakWorker);
            }else{
                workableWorkers.erase(currWorker);
            }
        }
        return true;
    }
