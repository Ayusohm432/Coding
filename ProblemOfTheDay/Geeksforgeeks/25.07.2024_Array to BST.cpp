...
class Solution {
  public:
    Node* balanceHeight(vector<int>& nums , int low , int high){
        if(low > high){
            return nullptr;
        }
        ...
        node->left = balanceHeight(nums , low , mid-1);
        node->right = balanceHeight(nums , mid+1 , high);
    ...
    Node* sortedArrayToBST(vector<int>& nums) {
        ...
        Node* root =  balanceHeight(nums , low , high);
        return root;
    }
};
